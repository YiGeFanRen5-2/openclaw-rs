...
impl OpenAIProvider {
    pub async fn new(config: &ProviderConfig) -> Result<Self, ProviderError> {
        let api_key = config.api_key.clone().ok_or_else(|| {
            ProviderError::Config("OpenAI requires api_key".into())
        })?;
...
/// Synchronous factory for use in create_provider dispatch
pub fn create_provider(config: &crate::provider::config::ProviderConfig) -> Result<Box<dyn crate::provider::trait_def::Provider>, crate::provider::trait_def::ProviderError> {
    Ok(Box::new(futures::executor::block_on(OpenAIProvider::new(config))?))
}
